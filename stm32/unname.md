...
串口通信的缺点: 
UART只是对信号的时序做了规定但是没有规定电气特性，UART没有规定不同器件连接时连接器的标准。 UART直接使用处理器的电平，而不同的处理器的高低电平不同。
抗干扰能力差，通讯距离短
对此开发了RS232协议作为串口通信的标准. 规定了-5~-15表示1，5~15表示0
RS232电平电压较高，要求转换芯片 成本高  通信速度低 。
在软件层次没有区别，只是在硬件上面加了一些标准。
RS485传输距离更长，抗干扰能力更强。并且可以实现点对多通信。
RS485使用差分信号进行传输，抗干扰能力强
