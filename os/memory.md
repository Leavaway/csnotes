 ...
 ## 地址转换
  ...
  ## 内存空间的扩充
   ...
   3. 虚拟内存: 为了解决传统存储方式的问题比如一次性, 驻留性。根据时间局部性和空间局部性<br/>
       ...
       页面分配策略:<br/>
          驻留集: 指请求分页存储管理中给进程分配的物理块的集合。在采用了虚拟存储技术的系统中，驻留集大小一般小于进程的总大小<br/>
          固定分配/可变分配: 驻留集大小不变/可变<br/>
          局部置换/全局置换: 只能选自己进程的/可以选系统空闲的或者将别的进程的物理块置换到外存再分配给缺页进程<br/>
          可变分配+全局置换: 只要缺页就会分配物理块, 当空闲物理空间用完时会选中一个进程置换出它的物理块, 因为被选中进程的物理块会减少, 缺页率会增加<br/>
          可变分配+局部置换: 根据缺页的频率动态调整操作系统分配给进程的物理块<br/>
          调入时机: 预调页策略(根据局部性原理, 主要用于进程的首次调入)  请求调页策略(在进程运行时发生缺页才调入, 这种策略调入的页面一定会被访问到但是一次只能调入一页, I/O开销大)<br/>
          工作集: 指在某段时间间隔里，进程实际访问页面的集合，一般来说，驻留集大小不能小于工作集大小，否则进程运行过程中将频繁缺页(抖动现象)<br/>
 ## 内存映射文件（Memory-mapped file）
   def: 是一种将文件或其他数据结构映射到计算机内存的技术。这种技术允许程序直接在内存中操作文件的内容，而无需使用传统的文件I/O操作。通过内存映射文件，文件内容可以被当作普通的内存地址空间进行访问，这使得对文件的读写操作更加高效<br/>
   原理: 操作系统为文件分配一个内存区域，这个区域称为内存映射区域。文件内容会被加载到这个内存映射区域中。这个过程通常是按需加载的，也就是说，在实际访问文件内容时，数据才会从磁盘加载到内存中。程序可以通过指针直接访问内存映射区域中的内容，而无需使用文件I/O操作。这使得对文件的读写操作变得更加高效，因为这些操作可以直接在内存中完成<br/>
