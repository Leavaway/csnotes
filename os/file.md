...
### 文件的基本操作
   创建文件: create 系统调用<br/>
   删除文件: delete 系统调用<br/>
   打开文件: open 系统调用, OS从磁盘中找到文件并且检查用户是否有相应的操作权限, 随后将目录项复制到用户的"打开文件表"和系统的"打开文件表", 系统的打开文件表里面有打开计数等参数进行文件管理<br/>
   读/写文件: read/write 系统调用<br/>
### 文件共享
   基于索引节点的共享方式: 1) 硬链接: 硬链接是一个指向文件数据的指针。在Unix和Linux系统中，每个文件至少有一个硬链接，它是文件名和文件数据之间的连接。硬链接实际上是指向文件的i节点的指针。i节点是存储在硬盘上的数据结构，其中包含了文件的元数据（如文件大小、创建和修改日期等）以及指向文件数据的指针 <br/>
   2) 软链接: 符号链接或软链接，相当于文件的快捷方式。它实际上是一个特殊的文件，该文件中包含了另一个文件或目录的路径名。软链接可以跨越文件系统，也可以链接到目录。<br/>
### 文件保护
   口令保护: 为文件设置口令在用户访问时要求提供。优点:保存口令的空间开销不多，验证口令的时间开销也很小。缺点:正确的“口令”存放在系统内部，不够安全<br/>
   加密保护: 使用特定的加密算法对文件进行加密。优点:保密性强，不需要在系统中存储“密码缺点:编码/译码，或者说加密/解密要花费一定时间<br/>
   访问控制: 在文件的fcb或者索引结点添加访问控制表(Access-Control List)<br/>
