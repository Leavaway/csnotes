...
## 浮点数表示
IEEE 754标准定义了浮点数的表示，这种表示包括符号位、指数位和尾数位。根据这个标准，浮点数可以分为规范化（normalized）和非规范化（denormalized）<br/>
规范化（Normalized）数：在这种情况下，二进制小数的第一位（在小数点之前）总是1。例如，1.101、1.01等。因为这个1总是存在，所以在IEEE 754浮点数表示中，这个1实际上是被省略的，以节省存储空间。这就是所谓的"隐藏的1"。例如，二进制数1.101在IEEE 754中表示为(隐藏的1).101。<br/>
非规范化（Denormalized）数：非规范化数是为了表示接近于零的非常小的数。对于规范化数，由于存在"隐藏的1"，我们无法表示小于1的非常小的数（比如0.0001）。但是，对于非规范化数，我们取消了这个"隐藏的1"，使得我们可以表示接近于零的非常小的数。这样，我们就可以表示一个范围更大的数值。<br/>
对于N, E = exp - bias  对于d, E = 1 - bias<br/>
Example: s exp4位 frac 3位   0 0000 000表示0    0 0000 001因为指数位全部为0, 表示为非规范化数, 为0.001, 指数为1 - 7 = -6, 表示1/512; 0 0110 110为规范化数, 指数为6 - 7 = -1, 表示1.110\*2^-1 = 7/8<br/>
(3.14+1e10)-1e10 = 0，3.14+(le10-le10) = 3.14 解释: 在第一个进行3.14和1e10运算时, 因为3.14的指数相比于1e10过小, 在指数对齐时, 3.14会变成0.000...1这样的情况, 然后在求和过程中它表示实际数据的位可能因为frac有限的长度被舍去, 所以3.14 + 1e10得到的结果还是1e10<br/>
